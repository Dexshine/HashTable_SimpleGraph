public class HashTable {
    ...
    public int getIndex(String s){
        int index = polyHash(s, this.p, this.x) % this.capacity;
        if(this.arr[index] == null) return index;
        else if(this.arr[index].equals(s)) return index;
        else if(this.arr[index] != null){
            for(int k=0; k<this.capacity; k++){
                index = (index + k) % this.capacity;
                if(this.arr[index] == null) return index;
                else if(this.arr[index].equals(s)) return index;
            }
        }
